...
async function getUserContacts(UID, socket, contactType) {
  ...
  if (!UID) return false
  if (!await databaseHandler.instances.user.functions.isUserExisting(UID)) return false
  if (contactType && (contactTypes.indexOf(contactType) == -1)) return false

  if (contactType) {
    var queryResult = await databaseHandler.server.query(`SELECT * FROM ${databaseHandler.instances.user.functions.getDependencyREF("contacts", UID)} WHERE type = '${contactType}'`)
    return (queryResult && queryResult.rows) || false
  }
  var queryResult = await databaseHandler.server.query(`SELECT * FROM ${databaseHandler.instances.user.functions.getDependencyREF("contacts", UID)}`)
...
eventServer.on("App:onClientConnect", function(socket, UID) {
  socket.on("App:Contacts:onClientFriendRequest", async function(UID, requestType) {
    ...
    if (requestType == "send") {
      ...
      queryResult = databaseHandler.utilities.fetchSoloResult(queryResult)
      if (queryResult) {
      ...
      var preparedQuery = databaseHandler.utilities.prepareQuery({
        UID: CInstances.UID,
        type: "pending"
      })
      ...
      if (requestType == "accept") {
        ...
        preparedQuery = databaseHandler.utilities.prepareQuery({
          UID: CInstances.UID,
          type: "friends",
          group: groupUID
        })
